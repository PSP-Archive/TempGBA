...
#define GPSP_CONFIG_NUM       (11 + 16) // options + game pad config
#define GPSP_GAME_CONFIG_NUM  (3)
...
s32 load_file(const char **wildcards, char *result, char *default_dir_name)
{
  ...
  while (return_value == 1)
  {
    ...
    while (repeat)
    {
      ...
      sprintf(str_buffer_size, "ROM Buffer %2dMB", gamepak_ram_buffer_size >> 20);
...
u32 menu(void)
{
  ...
  MenuOptionType emulator_options[] =
  {
    ...
    STRING_SELECTION_OPTION(NULL, MSG[MSG_OPTION_MENU_3], frameskip_options, &option_frameskip_type, 3, MSG_OPTION_MENU_HELP_3, 4),

    NUMERIC_SELECTION_OPTION(NULL, MSG[MSG_OPTION_MENU_4], &option_frameskip_value, 10, MSG_OPTION_MENU_HELP_4, 5),
    ...
    STRING_SELECTION_OPTION(NULL, MSG[MSG_OPTION_MENU_10], language_option, &option_language, 2, MSG_OPTION_MENU_HELP_10, 14), 

    ACTION_SUBMENU_OPTION(NULL, menu_init, MSG[MSG_OPTION_MENU_11], MSG_OPTION_MENU_HELP_11, 16)
  };
  ...
  MenuOptionType main_options[] =
  {
    ...
    ACTION_OPTION(menu_load_file, NULL, MSG[MSG_MAIN_MENU_7], MSG_MAIN_MENU_HELP_7, 10),

    ACTION_OPTION(menu_reset, NULL, MSG[MSG_MAIN_MENU_8], MSG_MAIN_MENU_HELP_8, 12),

    ACTION_OPTION(menu_exit, NULL, MSG[MSG_MAIN_MENU_9], MSG_MAIN_MENU_HELP_9, 13),

    ACTION_OPTION(menu_suspend, NULL, MSG[MSG_MAIN_MENU_10], MSG_MAIN_MENU_HELP_10, 15),

    ACTION_OPTION(menu_quit, NULL, MSG[MSG_MAIN_MENU_11], MSG_MAIN_MENU_HELP_11, 16)
  };
...
static void update_status_string(char *time_str, char *batt_str, u16 *color_batt)
{
  ...
  {
    sprintf(batt_str, "%s%s", batt_str, "[DC IN]");
  }
  else
  {
...
static s32 save_game_config_file(void)
{
  ...
  if (FILE_CHECK_VALID(game_config_file))
  {
    ...
    file_options[2] = option_clock_speed;
...
s32 save_config_file(void)
{
  ...
  if (FILE_CHECK_VALID(config_file))
  {
    ...
    file_options[3]  = option_sound_volume;
    file_options[4]  = option_stack_optimize;
    file_options[5]  = option_boot_mode;
    file_options[6]  = option_update_backup;
    file_options[7]  = option_screen_capture_format;
    file_options[8]  = option_enable_analog;
    file_options[9]  = option_analog_sensitivity;
    file_options[10] = option_language;

    for (i = 0; i < 16; i++)
    {
      file_options[11 + i] = gamepad_config_map[i];
    }
...
s32 load_game_config_file(void)
{
  ...
  if (FILE_CHECK_VALID(game_config_file))
  {
    ...
    if (file_size == (GPSP_GAME_CONFIG_NUM * 4))
    {
      ...
      option_clock_speed     = file_options[2] % 4;
...
s32 load_config_file(void)
{
  ...
  if (FILE_CHECK_VALID(config_file))
  {
    ...
    if (file_size == (GPSP_CONFIG_NUM * 4))
    {
      ...
      option_sound_volume   = file_options[3] % 11;
      option_stack_optimize = file_options[4] % 2;
      option_boot_mode      = file_options[5] % 2;
      option_update_backup  = file_options[6] % 2;
      option_screen_capture_format = file_options[7] % 2;
      option_enable_analog  = file_options[8] % 2;
      option_analog_sensitivity = file_options[9] % 10;
      option_language       = file_options[10] % 2;

      for (i = 0; i < 16; i++)
      {
        gamepad_config_map[i] = file_options[11 + i] % (BUTTON_ID_NONE + 1);
  ...
  void set_directory(char *dir_name, const char *item_name)
  {
    if (strcasecmp(current_variable, item_name) == 0)
    {
      ...
      else
      {
        ...
        print_string(str_buf, 6, str_line, COLOR15_WHITE, COLOR15_BLACK);
        str_line += FONTHEIGHT;
