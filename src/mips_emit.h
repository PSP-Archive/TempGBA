...
#define check_store_reg_pc_no_flags(reg_index)                                \
  if (reg_index == REG_PC)                                                    \
  {                                                                           \
    generate_function_call(execute_branch_ticks_arm);                         \
...
#define arm_multiply_long_flags_yes(_rdlo, _rdhi)                             \
  ...
  mips_emit_ext(reg_n_cache, _rdhi, 31, 1);                                   \
...
#define arm_block_memory_load_psr_no()                                        \
  mips_emit_jal(mips_absolute_offset(execute_branch_ticks_arm));              \
  generate_mov(reg_a0, reg_rv);                                               \
  generate_indirect_branch_cycle_update(arm);                                 \
...
{                                                                             \
  ...
  generate_load_reg(reg_a0, rn);                                              \
  mips_emit_sw(reg_rv, reg_base, (REG_TMP << 2));                             \
                                                                              \
  generate_load_pc(reg_a2, (pc + 4));                                         \
  generate_load_reg(reg_a0, rn);                                              \
  generate_load_reg(reg_a1, rm);                                              \
  mips_emit_lw(arm_to_mips_reg[rd], reg_base, (REG_TMP << 2));                \
...
#define thumb_block_memory_extra_pop_pc(access_type)                          \
  ...
  mips_emit_addiu(reg_a0, reg_a2, (bit_count[reg_list] << 2));                \
                                                                              \
  mips_emit_jal(mips_absolute_offset(execute_branch_ticks_thumb));            \
  generate_mov(reg_a0, reg_rv);                                               \
  generate_indirect_branch_cycle_update(thumb);                               \
...
{                                                                             \
  ...
  generate_condition_##condition();                                           \
                                                                              \
  generate_branch_no_cycle_update(thumb, block_exits[block_exit_position].branch_source, block_exits[block_exit_position].branch_target); \
                                                                              \
...
{                                                                             \
  ...
  generate_function_call_swap_delay(execute_branch_ticks_dual);               \
...
{                                                                             \
  ...
  generate_load_pc(reg_r14, ((pc + 2) | 0x01));                               \
  generate_function_call_swap_delay(execute_branch_ticks_thumb);              \
  generate_indirect_branch_cycle_update(thumb);                               \
}                                                                             \
